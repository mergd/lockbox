...
lockbox_setup() {
  ...
  if [[ -f "${LOCKBOX_AGE_KEY_FILE:-}" ]]; then
    key_material="$(cat "$LOCKBOX_AGE_KEY_FILE")"
    pubkey="$(printf '%s' "$key_material" | public_key_from_private)"
    echo "Using local age key: $LOCKBOX_AGE_KEY_FILE"
  else
    require_op
  ...
  write_private_key_cache "$key_material"
  ensure_sops_yaml "$pubkey"

  echo ""
  echo "Ready."
  echo "  1Password: $SOPS_AGE_KEY_OP_REF"
  echo "  Local key: $LOCKBOX_AGE_KEY_FILE"
